...
export default class extends think.model.mongo {
    ...
    //更新恐龙谷用户数据
    async updateKlgUserData(udata) {
        await this.where({ uname: udata.open_id }).update({
            $set: {
                "userData.name": udata.username, "userData.iconUrl": udata.avatar, "userData.sex": udata.sex, "userData.province": udata.province
                , "userData.city": udata.city
            }
        });
        var userdata = await this.where({ uname: udata.open_id }).find();
        return userdata;
    }
    //查找用户
    ...
    async updateUserRecord(udata,roomtype) {
        ...
        await this.where({ "userData.uid": Number(udata.uid) }).update({
            ...
            $push: { 'userData.recordData.recordList': { time: new Date().toLocaleString(), resultList: recordData.resultList, roomId: recordData.roomId,paybackId:recordData.paybackId } }
        });
        var userdata = await this.where({ "userData.uid": Number(udata.uid) }).find();
        if(userdata.userData.recordData.recordList.length > 50)
        {
            await this.where({ "userData.uid": Number(udata.uid) }).update({
                $pop: { "userData.recordData.recordList": -1 } 
            });
        }
    }
    //获取游戏战绩记录
